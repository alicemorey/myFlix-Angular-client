...
var UserRegistrationService = /** @class */ (function () {
    ...
    UserRegistrationService.prototype.getFavoriteMovies = function () {
        ...
        }).pipe(operators_2.map(function (response) { return response.FavoriteMovies; }), operators_1.catchError(this.handleError));
    };
    //add favorite movie endpoint
    UserRegistrationService.prototype.addFavoriteMovie = function (movieId) {
        var token = localStorage.getItem('token');
        var userId = localStorage.getItem('user_id'); // Assuming you store the user ID
        return this.http.post(apiUrl + "users/" + userId + "/movies/" + movieId, {}, // Empty body, as we're adding to favorites
        {
            headers: new http_1.HttpHeaders({
                Authorization: 'Bearer ' + token
            })
